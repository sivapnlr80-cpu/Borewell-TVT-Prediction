...
# ── Cell 0: Title & Specification ─────────────────────────────────────────────
cells.append(md(
"""# RSNA Knee Abnormality Detection 2026 — Multi-View KneeNet & ASL Pipeline

### Solution Architecture:
1. **Multi-Lingual Report Label Extractor**: Zero-shot & pattern extraction module to derive 12 binary target signals from multilingual reports.
2. **`KneeMRIDataset`**: Volume intensity normalization via 1st/99th percentile clipping, plane grouping ('Sagittal', 'Coronal', 'Axial'), and slice sampling.
3. **`MultiViewKneeNet` Architecture**: 2D slice-wise feature extraction (`convnext_small` / `tf_efficientnet_b4`), Spatial Attention Pooling across Z-dimension, and multi-plane fusion.
4. **Asymmetric Loss (ASL) & Stratified Multi-Label CV**: Custom ASL loss to handle severe label imbalance, mixed-precision AMP, and Cosine Annealing learning rate scheduling.
5. **100% Offline Safety**: Enforces `HF_HUB_OFFLINE=1`, `TIMM_OFFLINE=1`, and `pretrained=False` with local `state_dict` loading.
"""
))
...
for pkg in ['timm', 'albumentations', 'pydicom', 'accelerate', 'safetensors', 'iterative-stratification', 'scikit-learn']:
    try:
        ...
        for root, _, files in os.walk(sdir):
            for f in files:
                if f.endswith(('.pth', '.pt', '.bin', '.safetensors')):
                    if any(k in f.lower() or k in root.lower() for k in ['convnext', 'efficientnet', 'b4', 'timm', 'model', 'weight']):
...
CFG = dict(
    ...
    weight_decay     = 1e-2,
    device           = DEVICE,
    model_name       = 'convnext_small', # Fallback to 'tf_efficientnet_b4'
...
# ── Cell 3: Multi-Lingual Report Label Extractor ──────────────────────────────
cells.append(code(
"""# ── 3. Multi-Lingual Report Label Extractor ───────────────────────────────
class MultilingualReportExtractor:
    \"\"\"
    Extracts 12 binary target signals from raw radiology text reports.
    Handles foreign languages, negations ('no tear', 'intact'), and missing values.
    \"\"\"
    def __init__(self, target_cols=OFFICIAL_LABEL_COLS):
        self.target_cols = target_cols
        # Key clinical patterns per pathology
        self.patterns = {
            'ACL': [r'acl', r'anterior cruciate', r'kreuzband', r'ligamento cruzado anterior'],
            'MCL': [r'mcl', r'medial collateral', r'innenband'],
            'Medial Meniscus': [r'medial meniscus', r'innenmeniskus', r'menisco medial'],
            'Lateral Meniscus': [r'lateral meniscus', r'außenmeniskus', r'menisco lateral'],
            'Medial OA': [r'medial.*osteoarthr', r'medial.*arthros', r'gonarthros'],
            'Lateral OA': [r'lateral.*osteoarthr', r'lateral.*arthros'],
            'PF OA': [r'patellofemoral', r'patella.*arthros', r'retropatellar'],
            'Effusion': [r'effusion', r'erguss', r'derram'],
            'Synovitis': [r'synovitis', r'synovial'],
            "Baker's": [r'baker', r'popliteal cyst', r'poplitealzyste'],
            'Contusion': [r'contusion', r'bone bruise', r'knochenmarködem'],
            'Fracture': [r'fracture', r'fraktur', r'fractura']
        }
        self.negations = [r'no ', r'without', r'intact', r'unremarkable', r'kein', r'ohne', r'normal', r'sin ']

    def extract_labels_from_text(self, text: str) -> np.ndarray:
        if not isinstance(text, str) or not text.strip():
            return np.zeros(len(self.target_cols), dtype=np.float32)
            
        text_clean = text.lower()
        results = np.zeros(len(self.target_cols), dtype=np.float32)
        
        for i, col in enumerate(self.target_cols):
            pats = self.patterns.get(col, [re.escape(col.lower())])
            for pat in pats:
                match = re.search(pat, text_clean)
                if match:
                    start_pos = max(0, match.start() - 30)
                    context = text_clean[start_pos:match.start()]
                    is_neg = any(re.search(neg, context) for neg in self.negations)
                    if not is_neg:
                        results[i] = 1.0
                        break
        return results

extractor = MultilingualReportExtractor()
sample_text = "Intact ACL. High-grade tear of the medial meniscus. No joint effusion."
print("Sample extraction:", dict(zip(OFFICIAL_LABEL_COLS, extractor.extract_labels_from_text(sample_text))))
"""
))

# ── Cell 4: Data Schema & Series Mapping ──────────────────────────────────────
cells.append(code(
"""# ── 4. Data Schema & DICOM Series Mapping ─────────────────────────────────
...
series_csv_path = find_file('train_series.csv')
...
# ── Cell 5: DICOM Indexer & Plane Grouping ────────────────────────────────────
cells.append(code(
"""# ── 5. DICOM Indexer & Plane Grouping ('Sagittal', 'Coronal', 'Axial') ────
...
STUDY_SERIES_MAP = {}
print('Indexing DICOM series & anatomical planes...')
t0 = time.time()
...
if os.path.exists(input_dir):
    for root, _, files in os.walk(input_dir):
        ...
        # Route anatomical plane by folder naming heuristic
...
print(f'Indexed {len(STUDY_SERIES_MAP)} study mappings in {time.time()-t0:.2f}s')
...
# ── Cell 6: Custom PyTorch Dataset Class (`KneeMRIDataset`) ───────────────────
cells.append(code(
"""# ── 6. Custom PyTorch Dataset Class (`KneeMRIDataset`) ─────────────────────
def read_dicom_percentile(path: str, img_size: int = CFG['img_size']) -> np.ndarray:
    \"\"\"
    Percentile intensity windowing (1st to 99th percentile) to normalize 
    scanner gain and field strength variations (1.5T vs 3.0T).
    \"\"\"
    try:
...
class KneeMRIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load 3D volumes for Sagittal, Coronal, and Axial planes
...
# ── Cell 7: Neural Network Architecture (`MultiViewKneeNet`) ───────────────────
cells.append(code(
"""# ── 7. Neural Network Architecture (`MultiViewKneeNet`) ───────────────────
...
class MultiViewKneeNet(nn.Module):
    \"\"\"
    Multi-View KneeNet: Accepts Sagittal, Coronal, and Axial volumes.
    Applies 2D backbone slice-wise + 1D Spatial Attention Pooling per plane,
    concatenates multi-plane representations with tabular metadata, and outputs 12 logits.
    \"\"\"
    def __init__(self, n_classes=len(LABEL_COLS), model_name=CFG['model_name'], weights_path=CFG['img_weights_path']):
        ...
        for bname in ['convnext_small', 'tf_efficientnet_b4.ns_jft_in1k', 'tf_efficientnet_b4', 'resnet34d', 'resnet34']:
            try:
                self.backbone = timm.create_model(bname, pretrained=False, num_classes=0)
                print(f'Created MultiViewKneeNet backbone: {bname} (pretrained=False)')
                backbone_built = True
                break
            except Exception:
                continue
        ...
        fused_dim = in_feat * 3 + 2 # 3 planes + 2 tabular features
...
def build_knee_model(n_classes=len(LABEL_COLS)):
    ...
    print(f'MultiViewKneeNet built | Parameters: {trainable/1e6:.2f}M / {total/1e6:.2f}M')
    return model
"""
))

# ── Cell 8: Asymmetric Loss (ASL) & Training Engine ───────────────────────────
cells.append(code(
"""# ── 8. Asymmetric Loss (ASL) & Training Engine ──────────────────────────────
...
# ── Cell 9: Multilabel Stratified K-Fold Training Loop ────────────────────────
cells.append(code(
"""# ── 9. Multilabel Stratified K-Fold Loop ───────────────────────────────────
def run_fold(fold, tr_df, vl_df):
    ...
    for epoch in range(1, CFG['n_epochs'] + 1):
        ...
        print(f'Epoch {epoch}/{CFG["n_epochs"]} [{elapsed:.1f}s] | Train Loss: {tr_loss:.4f} | Val Loss: {vl_loss:.4f} | Val AUC: {vl_auc:.4f} | Val F1: {vl_f1:.4f}')
...
# Multilabel Stratified Split with Safety Fallback
if HAS_ITERSTRAT and all(c in train_df.columns for c in LABEL_COLS):
    mskf = MultilabelStratifiedKFold(n_splits=CFG['n_folds'], shuffle=True, random_state=SEED)
    Y_mat = train_df[LABEL_COLS].fillna(0).values
    splits = list(mskf.split(train_df, Y_mat))
else:
...
# ── Cell 10: Multi-View Inference & Submission Generator ─────────────────────
cells.append(code(
"""# ── 10. Multi-View Inference & Official Submission Generator ───────────────
@torch.no_grad()
...
print(f'Official submission generated successfully at {out_file} (shape: {sub_df.shape})')
...
print("Generated rsna_knee_solution.ipynb (v5 MultiViewKneeNet) successfully!")
