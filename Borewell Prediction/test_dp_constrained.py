...
def run_constrained_hybrid(h_df, t_df, n_states=100, smoothness=50.0, window_width=4.0, max_d_offset=0.05):
    # Resolve columns
    tw_depth_col = 'MD'
    tw_gr_col = 'GR'
    h_gr_col = 'GR'
    ...
    # 1. Fit Ridge regression on known section
    known_df = h_df.iloc[:eval_start]
    X_train = known_df[[h_md_col, h_z_col]]
    ...
    lr = Ridge(alpha=10.0).fit(X_train_scaled, y_train)
    
    # Predict trend for the entire well
    all_md_z = h_df[[h_md_col, h_z_col]].values
    all_md_z_scaled = (all_md_z - mean.values) / std.values
    tvt_trend = lr.predict(all_md_z_scaled)
...
for f in sorted(glob.glob('train/*_horizontal_well.csv')):
    ...
    pred = run_constrained_hybrid(h_df, t_df)
    rmse = np.sqrt(np.mean((pred - true_tvt)**2))
    rmses.append(rmse)
    print(f"Well {wellname}: Constrained Hybrid RMSE = {rmse:.4f}")

print(f"\nAverage Constrained Hybrid RMSE = {np.mean(rmses):.4f}")
